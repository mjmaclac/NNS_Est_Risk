function [ax,hlines] = plotyyy(h,x1,y1,x2,y2,x3,y3,ylabels)
...
[ax,hlines(1),hlines(2)] = plotyy(x1,y1,x2,y2,@area,@line);
set(hlines(1), 'FaceColor', 'k','FaceAlpha', 0.1,'EdgeAlpha',0.1);
set(hlines(2), 'LineWidth',1.15);
tick_vec=[0 1 1e2 1e6 1e8 1e9];
set (ax(1), 'Yscale', 'log','Ytick',tick_vec,'Yticklabels',string(tick_vec*1e-6));
xlim(ax(2), [min(x1) max(x1)])
ylim(ax(2),[0 1.03*round(max(y2),2,'significant')])
set(ax(1),'Box','off')
set(ax(2),'Box','off')
set(ax(2),'Ytick',[0 round(max(y2)/3,2,'significant') round(2*max(y2)/3,2,'significant') round(max(y2),2,'significant')])
hlines(2).LineStyle = '--';
cfig = get(gcf,'color');
pos = get(h,'Position');
offset = pos(3)/5.5;
...
limx1=get(ax(2),'xlim');
limx3=[limx1(1)  limx1(2)*1.0155]; %limx1(1)+ 0.75*(limx1(2)-limx1(1))];
...
hlines(3) = line(x3,y3,'Color','r','Parent',ax(3),'LineWidth',1.25);
limy3=get(ax(3),'YLim');
